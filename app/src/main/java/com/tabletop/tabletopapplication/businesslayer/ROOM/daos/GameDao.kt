...
interface GameDao {
    ...
    @Query("SELECT n.noteDescription,n.gameId,n.id FROM gameTable AS g JOIN notesTable AS n ON g.id = n.gameId WHERE n.gameId = :id")
    ...
    @Query("Select h.id,h.gameId from gameTable as g join hourglassTable as h on g.id=h.gameId Where h.gameId=:id ")
    fun getHourglassesOfGame(id: Int): Flow<List<HourglassROOM>>

    @Query("Select n.noteDescription,n.gameId,n.id from gameTable as g join notesTable as n on g.id=n.gameId Where n.gameId=:gameid AND n.id=:noteid Order BY n.id ")
    ...
    @Query("Select h.id,h.gameId from gameTable as g join hourglassTable as h on g.id=h.gameId Where h.gameId=:gameid AND h.id=:hourglassid Order BY h.id")
